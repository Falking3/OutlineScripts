...
public class OutlinePostProcessorUVMethod : AssetPostprocessor
{
    ...
    void OnPostprocessModel(GameObject gameObject) 
    {
        ...
        {
            Debug.Log($"Baking normals to UV3 for model '{gameObject.name}'");
    ...
    void Apply(GameObject gameObject, string assetPath)
    {
        ...
        {
            Mesh ogmesh = mf.sharedMesh; //sharedMesh stops it from duplicating, need to read further on this
            ...
            foreach (Vector3 coords in ogmesh.vertices)
            {
                int index = vertcoords_to_new_index[coords];
                var normalvalue = newmesh.normals[index];
                newnormals.Add(normalvalue);
            }

            mf.sharedMesh.SetUVs(3, newnormals);
